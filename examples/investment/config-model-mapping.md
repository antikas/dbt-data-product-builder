# Config-Model Mapping: curated.fund_master
...
| 8 | data_validation - post | Tests in `_cur_fund_master__models.yml` - 2 singular tests | n/a |
| 9 | data_contracts | Contract in `_cur_fund_master__models.yml` | n/a |
| 10 | lineage_capture | This document + dbt native DAG | n/a |
| 11 | metadata_capture | `meta:` tags in `_cur_fund_master__models.yml` | n/a |
| 12 | schema_publish | `access: public`, `latest_version: 1` in `_cur_fund_master__models.yml` | n/a |
| 13 | data_publish | Build guidance in `cur_fund_master.sql` header + grants + post_hook | n/a |
| **Final** | - | `cur_fund_master.sql` | incremental |
...
| Curated | `fund_id` | `cur_fund_master` | Pass-through |
...
| Curated | `fund_name` | `cur_fund_master` | Pass-through |
...
| Curated | `legal_name` | `cur_fund_master` | Pass-through |
...
| Curated | `strategy_name` | `cur_fund_master` | Pass-through |
...
| Curated | `tvpi` | `cur_fund_master` | Pass-through |
...
| Curated | `has_data_quality_flag` | `cur_fund_master` | Pass-through |
...
| Curated | `usd_fx_rate` | `cur_fund_master` | Pass-through |
...
| fund_id uniqueness (post) | `unique` | `_cur_fund_master__models.yml` | error |
| fund_id completeness (post) | `not_null` | `_cur_fund_master__models.yml` | error |
| fund_name completeness (post) | `not_null` (threshold 0.95) | `_cur_fund_master__models.yml` | warn |
| committed_capital range (post) | `accepted_range` | `_cur_fund_master__models.yml` | error |
...
- Two schema_transforms (one per source) feed into a single data_curation step, following the multi-source CDP pattern.
...
- The quarantine model captures records failing error-severity post-validation rules before they reach the curated model.
