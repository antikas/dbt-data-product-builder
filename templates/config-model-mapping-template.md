...
| Step 9 | data_validation - post | Tests in `_cur_{entity}__models.yml` | n/a |
| Step 10 | data_contracts | Contract in `_cur_{entity}__models.yml` | n/a |
| Step 11 | lineage_capture | This document + dbt native DAG | n/a |
| Step 12 | metadata_capture | `meta:` tags in schema.yml files | n/a |
| Step 13 | schema_publish | `access:` in `_cur_{entity}__models.yml` | n/a |
| Step 14 | data_publish | Build guidance in model comments | n/a |
| **Final** | - | `cur_{entity}.sql` | table |
...
| Curated | `{target_field}` | `cur_{entity}` | Final output |
...
| Curated | `{field_name}` | `cur_{entity}` | Final output |
...
- The mapping doc provides **field-level** lineage that dbt's DAG does not capture.
